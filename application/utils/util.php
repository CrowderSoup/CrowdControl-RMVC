...
if ( ! class_exists( 'util' ) ) {
    class util
    {
        ...
        public static function human_time_diff( $from, $to = '', $as_text = FALSE, $suffix = ' ago' )
        {
            ...
            if ($diff <= 1) {
                $text = '1 second';
            } elseif ($diff <= self::SECONDS_IN_A_MINUTE) {
                $text = $diff . ' seconds';
            } elseif ( $diff < ( self::SECONDS_IN_A_MINUTE * 2 ) ) {
                $text = '1 minute';
            } elseif ($diff < self::SECONDS_IN_AN_HOUR) {
                $text = floor( $diff / self::SECONDS_IN_A_MINUTE ) . ' minutes';
            } elseif ( $diff < ( self::SECONDS_IN_AN_HOUR * 2 ) ) {
                $text = '1 hour';
            } elseif ($diff < self::SECONDS_IN_A_DAY) {
                $text = floor( $diff / self::SECONDS_IN_AN_HOUR ) . ' hours';
            } elseif ( $diff < ( self::SECONDS_IN_A_DAY * 2 ) ) {
                $text = '1 day';
            } elseif ($diff < self::SECONDS_IN_A_WEEK) {
                $text = floor( $diff / self::SECONDS_IN_A_DAY ) . ' days';
            } elseif ( $diff < ( self::SECONDS_IN_A_WEEK * 2 ) ) {
                $text = '1 week';
            } elseif ($diff < self::SECONDS_IN_A_MONTH) {
                $text = floor( $diff / self::SECONDS_IN_A_WEEK ) . ' weeks';
            } elseif ( $diff < ( self::SECONDS_IN_A_MONTH * 2 ) ) {
                $text = '1 month';
            } elseif ($diff < self::SECONDS_IN_A_YEAR) {
                $text = floor( $diff / self::SECONDS_IN_A_MONTH ) . ' months';
            } elseif ( $diff < ( self::SECONDS_IN_A_YEAR * 2 ) ) {
                $text = '1 year';
            } else {
